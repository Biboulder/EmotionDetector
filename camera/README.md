...
* `esp32`: This application initializes the camera, waits for an incoming 'S' character on the serial port, and then takes one 320x320 RGB565 photo every second and sends the raw image bytes over the serial port, preceded by the preamble sequence `"\n===FRAME===\n"`. The firmware captures a 480x320 sensor frame and center-crops it to 320x320 before transmission.
...
3. In the ESP-IDF terminal in VS Code, build, flash and run the application.
    ...
    ```
    I (1262) CAMERA: Camera initialized: 320x320 RGB565.
    Send 'S' to start.
    ```
